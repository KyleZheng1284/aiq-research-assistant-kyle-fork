...
- After successful `execute` calls, a manifest-only checkpoint harvest durably stores
  completed artifacts without scanning the whole workspace. Terminal finalization runs a
  final manifest + directory scan on success or failure and a bounded best-effort harvest
  before forced termination on cancellation/timeout.
- The `ArtifactManager` pulls bytes via `download_files`, runs the validation pipeline
  ...
  `artifact.update` SSE event (`data.type=file`, metadata + content URL, never bytes).
