...
class TestEventStore:
    ...
    def test_artifact_update_survives_event_store_round_trip(self, tmp_path):
        """The canonical file event remains reconstructable after durable storage."""
        ...
        assert event["data"]["content"] == content_url
...
class TestTerminalTeardown:
    """_teardown_sandbox prefers runtime finalization and preserves legacy fallbacks."""
    ...
    def test_runtime_finalizer_receives_terminal_state(self):
        from aiq_api.jobs.runner import _teardown_sandbox
    ...
    def test_runtime_finalizer_never_raises(self):
        from aiq_api.jobs.runner import _teardown_sandbox

        runtime = MagicMock(spec=["finalize"])
        runtime.finalize.side_effect = RuntimeError("finalizer failed")
