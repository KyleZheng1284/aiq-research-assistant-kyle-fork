...
from aiq_agent.agents.deep_researcher.sandbox.providers.openshell import _load_policy_proto
...
@contextmanager
def _fake_optional_modules(context_factory):
    ...
    adapter_module.OpenShellSandbox = _FakeAdapter  # type: ignore[attr-defined]
...
def test_per_job_session_uses_policy_spec_and_attests_before_return(tmp_path: Path) -> None:
    ...
    with (
        ...
        patch(
            "aiq_agent.agents.deep_researcher.sandbox.providers.openshell._load_policy_proto",
...
def test_policy_denial_emits_sanitized_event_without_command() -> None:
    provider = _provider()
    provider._session.execute.return_value = SimpleNamespace(  # type: ignore[union-attr]
    ...
    assert events == [
        {
            ...
            "data": {"provider": "openshell", "sandbox": provider.sandbox_name, "exit_code": 1},
        }
    ]
